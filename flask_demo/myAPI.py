...
@app.route('/myAPI', methods=['POST']) # for calling the API from Postman/SOAPUI
def myfunc():
    if (request.method=='POST'):
        ...
        if(operation=='add'):
            try:
                r = int(var1) + int(var2)
                result = 'the sum of ' + str(var1) + ' and ' + str(var2) + ' is ' + str(r)
                return jsonify(result)
            except Exception as e:
                return jsonify("Value Error Occurs. Please check the input and try again!!")
        if (operation == 'reverse'):
            try:
                r1 ="".join(reversed(var1))
                r2= "".join(reversed(var2))
                result = 'the reverse of both the strings are ' + str(r1) + ' and ' + str(r2)
                return jsonify(result)
            except Exception as e:
                return jsonify("Value Error Occurs. Please check the input and try again!!")
